...
export default function DesktopShowcase({ initialIndex = 0, onOpen, className }: ShowcaseProps) {
  ...
  const normalizedIndex =
    projectCount > 0 ? ((displayIndex % projectCount) + projectCount) % projectCount : 0;
  const active = projectCount > 0 ? visibleProjects[normalizedIndex] : undefined;
    ...
    const h = (e: KeyboardEvent) => {
      ...
      if (e.key === 'ArrowLeft') {
        ...
        if (prev >= 0) {
          ...
          const prevCount = getVisibleProjects(CATEGORIES[prevCat].projects).length;
